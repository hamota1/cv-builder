...
const ClassicBlueSidebarLayout = () => {
    ...
    const sections = [
        { id: 'contact', title: 'CONTACT', content: pInfo, hasData: pInfo.email || pInfo.phone || pInfo.LinkedIn || pInfo.location },
    ...
    return (
        <div className="cbs-v2-container">
            <div className="cbs-v2-left-sidebar">
                {pInfo.profilePicture && (
                    <div className="cbs-v2-profile-picture-container">
                        <img src={pInfo.profilePicture} alt="Profile" className="cbs-v2-profile-picture" />
                    </div>
                )}
                <div className="cbs-v2-personal-header">
                ...
                {filledSections.map((section) => (
                    (section.id === 'contact' || section.id === 'education' || section.id === 'skills' || section.id === 'languages' || section.id === 'certifications') && section.hasData && (
                        <section key={section.id} className="cbs-v2-section">
                            <h3>{section.title}</h3>
                            {section.id === 'contact' && (
                                <div className="cbs-v2-contact-info">
                                    {pInfo.email && <p><FaEnvelope className="cbs-v2-icon" /> {pInfo.email}</p>}
                                    {pInfo.phone && <p><FaPhone className="cbs-v2-icon" /> {pInfo.phone}</p>}
                                    {pInfo.LinkedIn && <p><FaLinkedin className="cbs-v2-icon" /> {pInfo.LinkedIn}</p>}
                                    {pInfo.location && <p><FaMapMarkerAlt className="cbs-v2-icon" /> {pInfo.location}</p>}
                                </div>
                            )}
                            {section.id === 'education' && (
                                <ul>
                                    {section.content.map((edu, i) => (
                                        <li key={i} className="cbs-v2-item">
                                            <h4>{edu.degree}{edu.major && `, ${edu.major}`}</h4>
                                            <p><strong>{edu.university}</strong> | {edu.year}</p>
                                            {edu.achievements && edu.achievements.length > 0 && (
                                                <ul className="cbs-v2-achievements-list">{edu.achievements.map((a, idx) => a && <li key={idx}>{a}</li>)}</ul>
                                            )}
                                        </li>
                                    ))}
                                </ul>
                            ...
                            {section.id === 'skills' && (
                                <ul className="cbs-v2-skills-list">
                                    {section.content.map((s, i) => s.skill && (
                                        <li key={i}>
                                            <span>{s.skill}</span>
                                            <div className="cbs-v2-skill-bar">
                                                <div 
                                                    className="cbs-v2-skill-level"
                            ...
                            {section.id === 'languages' && (
                                <ul className="cbs-v2-simple-list">
            ...
            <div className="cbs-v2-right-main-content">
                <div className="cbs-v2-main-header">
                    <h1>{pInfo.fullName}</h1>
                    <h2>{pInfo.jobTitle}</h2>
                </div>
                                    ...
                                    {section.content.map((exp, i) => (
                                        <li key={i} className="cbs-v2-item">
                                            ...
                                            {exp.achievements && exp.achievements.length > 0 && (
                                                <ul className="cbs-v2-achievements-list">{exp.achievements.map((a, idx) => a && <li key={idx}>{a}</li>)}</ul>
                                            )}
                                        </li>
                                    ))}
                                </ul>
                                    ...
                                    {section.content.map((p, i) => (
                                        <li key={i} className="cbs-v2-item">
                                            ...
                                            {p.achievements && p.achievements.length > 0 && (
                                                <ul className="cbs-v2-achievements-list">{p.achievements.map((a, idx) => a && <li key={idx}>{a}</li>)}</ul>
                                            )}
                                        </li>
                                    ))}
                                </ul>
                                    ...
                                    {section.content.map((v, i) => (
                                        <li key={i} className="cbs-v2-item">
                                            ...
                                            {v.achievements && v.achievements.length > 0 && (
                                                <ul className="cbs-v2-achievements-list">{v.achievements.map((a, idx) => a && <li key={idx}>{a}</li>)}</ul>
                                            )}
                                        </li>
                                    ))}
                                </ul>
...
export default ClassicBlueSidebarLayout;